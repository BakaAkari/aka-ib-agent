...
const logLevelSchema: Schema<LogLevel> = Schema.union([
  Schema.const('info').description('记录关键流程'),
  Schema.const('debug').description('记录详细调试信息'),
]).role('radio')
...
export const Config: Schema<Config> = Schema.intersect([
  Schema.object({
    ...
    logLevel: logLevelSchema.default('info').description('插件日志详细程度'),
...
export function apply(ctx: Context, config: Config) {
  ...
  const chatCommand = ctx.command(`${config.chatCommandName} [message:text]`, '将自然语言请求转发给 ibkr 分析服务')
    ...
    .action(async ({ options, session }, message) => {
      ...
      if (permissionError) {
        logInfo(logger, config, 'command denied, %o', sessionMeta)
      ...
      logInfo(logger, config, 'command request, mode=%s, %o', responseMode, sessionMeta)
      logDebug(logger, config, 'command payload preview=%s', truncateForLog(content))
      ...
      if (!result.ok) {
        logInfo(logger, config, 'command failed, %o', sessionMeta)
        return result.message
      }

      logInfo(logger, config, 'command success, intent=%s, level=%s, %o', result.data.intent, result.data.decision_level, sessionMeta)
      logDebug(logger, config, 'command response summary=%s', truncateForLog(result.data.summary))
  ...
  const healthCommand = ctx.command(`${config.chatCommandName}.health`, '检查 ibkr 服务健康状态')
    ...
    .action(async ({ session }) => {
      ...
      const permissionError = getPermissionError(session as SessionLike, config)
      if (permissionError) {
        logInfo(logger, config, 'health denied, %o', sessionMeta)
        return permissionError
      }

      logInfo(logger, config, 'health request, %o', sessionMeta)
      const result = await client.health()
      if (!result.ok) {
        logInfo(logger, config, 'health failed, %o', sessionMeta)
        return result.message
      }
  ...
  if (!config.enableMiddleware) {
    logger.info('enabled, baseUrl=%s, middleware=off, logLevel=%s', normalizeBaseUrl(config.baseUrl), config.logLevel)
    return
  }
  ...
  ctx.middleware(async (session, next) => {
    ...
    if (shouldSkipSession(session as SessionLike, config)) {
      logDebug(logger, config, 'middleware skipped, %o', sessionMeta)
      return next()
    }

    const permissionError = getPermissionError(session as SessionLike, config)
    if (permissionError) {
      logDebug(logger, config, 'middleware denied, %o', sessionMeta)
      return next()
    }

    if (looksLikeCommandInvocation(session.content || '', commandNames)) {
      logDebug(logger, config, 'middleware bypassed command invocation, %o', sessionMeta)
      return next()
    }

    const message = extractMiddlewareMessage(session as SessionLike, config)
    if (!message) {
      logDebug(logger, config, 'middleware no trigger match, %o', sessionMeta)
      return next()
    }

    logInfo(logger, config, 'middleware request, mode=%s, %o', config.middlewareResponseMode, sessionMeta)
    logDebug(logger, config, 'middleware payload preview=%s', truncateForLog(message))
    const result = await client.analyze({
      message,
      mode: 'chat',
    ...
    if (!result.ok) {
      logInfo(logger, config, 'middleware failed, %o', sessionMeta)
      await session.send(result.message)
      return
    }

    logInfo(logger, config, 'middleware success, intent=%s, level=%s, %o', result.data.intent, result.data.decision_level, sessionMeta)
    await session.send(renderResponse(result.data, config.showDiagnostics, session))
  })

  logger.info('enabled, baseUrl=%s, middleware=on, logLevel=%s', normalizeBaseUrl(config.baseUrl), config.logLevel)
}
...
class IbkrClient {
  ...
  async analyze(payload: AnalyzeRequest): Promise<{ ok: true, data: AnalyzeResponse } | { ok: false, message: string }> {
    const url = this.buildUrl('/api/v1/analyze')
    logDebug(this.logger, this.config, 'http post start, url=%s, mode=%s, response_mode=%s, allow_execution=%s', url, payload.mode, payload.response_mode, payload.allow_execution)
    try {
  ...
  async health(): Promise<{ ok: true, data: { status: string } } | { ok: false, message: string }> {
    ...
    logDebug(this.logger, this.config, 'http get start, url=%s', url)
    try {
...
function logInfo(logger: ReturnType<Context['logger']>, config: Config, message: string, ...args: unknown[]) {
  logger.info(message, ...args)
}

function logDebug(logger: ReturnType<Context['logger']>, config: Config, message: string, ...args: unknown[]) {
...
function getSessionMeta(session: SessionLike) {
  return {
    platform: session.platform,
    userId: session.userId,
    channelId: session.channelId,
    guildId: session.guildId,
    authority: getAuthority(session),
  }
}

function truncateForLog(text: string, limit = 120) {
...
function formatErrorForLog(error: unknown) {
  if (error && typeof error === 'object') {
    const candidate = error as { code?: string, message?: string, name?: string, response?: { status?: number } }
